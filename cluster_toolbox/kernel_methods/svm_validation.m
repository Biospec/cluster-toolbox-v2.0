function output = svm_validation(data, label, rep_idx)

% output = svm_validation(data, label, rep_idx)
% A double cross-validation procedure for linear support vector machines 
...
%         data_trn=data_trn-ones(no_sample_trn,1)*data_centre; 
        ...
        if length(unique(class_trn))==no_class
            i=i+1;
        else
            continue;
        end
...
%            delta_inner=zeros(size(class_val_inner));
           for iii=1:no_C_grid
               svm_str = ['-t 0 -c ' num2str(C(iii)) ' -q ' w_str];
               model = svmtrain(class_trn_inner, data_trn_inner, svm_str);
               class_val_inner(:,iii) = svmpredict(label_val, data_val, model, '-q');
           end
        ...
        C_opt_idx=find(ccr_cv==max(ccr_cv));
        if length(C_opt_idx)>1
            C_opt_idx=C_opt_idx(1);
        end
        C_opt=C(C_opt_idx);
        svm_str = ['-t 0 -c ' num2str(C_opt) ' -q ' w_str];
        ...
        for ii=1:no_class;
            for iii=1:no_class
                conf_mat(ii,iii)=length(find(class_tst == ii & ...
                    class_tst_pred == iii))/length(find(class_tst == ii));
            end
        end    
        for ii=1:no_class;
            for iii=1:no_class
                conf_mat2(ii,iii)=length(find(class_tst == ii &...
                    class_tst_pred2 == iii))/length(find(class_tst == ii));
            end
        end
