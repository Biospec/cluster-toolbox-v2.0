function output = rbfdualplsr_boots(data, conc, rep, no_loops)
% output = rbfdualplsr_boots(data, conc, rep, no_loops)
...
%                 svm_str = ['-s 3 -t 2 -p ' num2str(epsilon) ' -c ' ...
%                     num2str(C(a)) ' -g ' num2str(G(aa)) ' -q -v 7'];
%                 [T, val_error] = evalc('svmtrain(conc_trn, data_trn, svm_str)');
%                 idx = find(ismember(T,'='));
%                 Q2(a,aa) = str2num(T(idx(end)+1:end));
% %                 predC = svmpredict(conc_val,data_val, model, '-q');
% %                 predCval=[predCval; predC]; Cval=[Cval;conc_val];
% %             end
%             rmsecv(a,aa)=val_error;
%             end
%         end
        opt_parameter = dualplsr_tune(data_trn, conc_trn, 'rbf');
        opt_k = opt_parameter.opt_k; 
        opt_gamma = opt_parameter.opt_gamma;
        rmsecv_boots(i,:)=opt_parameter.opt_rmsecv;
        Q2_boots(i,:)=opt_parameter.opt_Q2;
        Xcentre = mean(data_trn);
        Ycentre = mean(conc_trn);
        ...
        [alpha, predC] = dualpls(Ktr, Kte, Ytrn, opt_k);
        [alpha, predC2] = dualpls(Ktr, Kte2, Ytrn, opt_k);
...
% auto-prediction statistics, such as VIP, Loadings, R2 and RMSEC
    opt_k_final=min(output.opt_k); opt_gamma_final = mean(output.opt_G);
    centre = mean(data);
    X = data - repmat(centre, size(data,1),1);
    Ktr = rbf_two(X, X, opt_gamma_final);
    [alpha, predCtrn] = dualpls(Ktr, Ktr, conc, opt_k_final);    
